  ...
  return <main className="noise overflow-hidden">
    <nav className="fixed top-0 z-50 w-full border-b border-white/10 bg-[#05070d]/75 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <a href="/enquiry" className="hidden rounded-full border border-white/15 bg-white/5 px-5 py-2.5 text-sm font-medium hover:bg-white/10 md:block">Start a conversation <ArrowUpRight className="ml-1 inline h-4 w-4" /></a>
        <button onClick={()=>setOpen(!open)} className="rounded-lg border border-white/10 p-2 md:hidden" aria-label="Menu">{open?<X/>:<Menu/>}</button>
      </div>
      {open && <div className="border-t border-white/10 px-6 py-5 md:hidden"><div className="flex flex-col gap-5 text-white/70"><a onClick={()=>setOpen(false)} href="#capabilities">Capabilities</a><a onClick={()=>setOpen(false)} href="#builds">What we build</a><a onClick={()=>setOpen(false)} href="#approach">Approach</a><a onClick={()=>setOpen(false)} href="#contact">Contact</a><a onClick={()=>setOpen(false)} href="/enquiry">Enquiry</a></div></div>}
    </nav>
    ...
    <section id="top" className="grid-bg relative flex min-h-screen items-center pt-24">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-14 px-6 py-24 lg:grid-cols-[1.05fr_.95fr]">
        <div>
          ...
          <div className="mt-9 flex flex-wrap gap-3"><a href="#capabilities" className="rounded-full bg-white px-6 py-3 text-sm font-semibold text-slate-950 hover:bg-cyan-100">Explore capabilities <ArrowUpRight className="ml-1 inline h-4 w-4"/></a><a href="/enquiry" className="rounded-full border border-white/15 bg-white/5 px-6 py-3 text-sm font-semibold hover:bg-white/10">Send enquiry</a></div>
